...
class WC_Polar_Gateway extends WC_Payment_Gateway {
	...
	public function process_payment( $order_id ) {
		...
		try {
			...
			if ( ! $checkout_url ) {
				throw new Exception( esc_html__( 'Unable to create checkout session with Polar.', 'apwg' ) );
			}
	...
	private function create_checkout_session( $order ) {
		if ( empty( $this->access_token ) ) {
			throw new Exception( esc_html__( 'Polar Access Token is not configured.', 'apwg' ) );
		}
		...
		$api_url = 'https://api.polar.sh/v1/checkouts/';
			...
			$this->log( 'Checkout data: ' . wp_json_encode( $checkout_data ) );
	 ...
	 * @param WC_Order $order Order object.
	 * @return array
	 * @throws Exception If mapping is missing.
	 */
			...
			switch ( $this->product_mapping ) {
				...
				case 'single':
					...
					if ( empty( $this->default_product_id ) ) {
						throw new Exception( esc_html__( 'Default Product ID is not configured.', 'apwg' ) );
					}
	 ...
	 * @throws Exception If subscription product missing Polar ID.
	 */
				...
				if ( empty( $polar_product_id ) ) {
					throw new Exception(
						sprintf(
							...
							esc_html__( 'Product "%s" is marked as subscription but has no Polar Product ID. Please configure it in the product settings.', 'apwg' ),
	 ...
	 * @param WC_Order $order Order object.
	 * @return array
	 * @throws Exception If default product ID is missing.
	 */
	private function prepare_single_dynamic( $order ) {
		if ( empty( $this->default_product_id ) ) {
			throw new Exception( esc_html__( 'Default Product ID is not configured. Please create a Product on Polar and enter its ID in Settings.', 'apwg' ) );
		}
	 ...
	 * @param WC_Order $order Order object.
	 * @return array
	 * @throws Exception If no Polar ID found in meta.
	 */
	private function prepare_from_meta( $order ) {
		...
		if ( empty( $product_ids ) ) {
			throw new Exception( esc_html__( 'No Polar Product ID found in products.', 'apwg' ) );
		}
	...
	private function handle_payment_success( $data ) {
		...
		if ( 'succeeded' === $status || 'order.created' === $data['type'] ) {
	...
	public function receipt_page( $order_id ) {
		...
		if ( $checkout_url ) {
			echo '<p>' . esc_html__( 'You will be redirected to Polar payment page...', 'apwg' ) . '</p>';
		...
		<div class="options_group">
			<p class="form-field">
				<label for="polar_is_subscription">
					...
					<?php esc_html_e( 'Enable Polar Subscription', 'apwg' ); ?>
				</label>
				<span class="description">
					<?php esc_html_e( 'Check this if this product should be sold as a subscription via Polar.', 'apwg' ); ?>
				</span>
			</p>
			
			<p class="form-field" id="polar_product_id_field">
				<label for="polar_product_id"><?php esc_html_e( 'Polar Product ID', 'apwg' ); ?></label>
				<input type="text" 
						id="polar_product_id" 
				...
				<span class="description">
					<?php esc_html_e( 'Enter the Polar Product ID for this product. Required if subscription is enabled or using "From Product Meta Field" mapping.', 'apwg' ); ?>
				</span>
			</p>
		</div>
	...
	public function save_product_meta( $post_id ) {
		if ( ! isset( $_POST['polar_product_meta_nonce'] ) ||
			! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['polar_product_meta_nonce'] ) ), 'polar_product_meta' ) ) {
			return;
		}
		...
		if ( isset( $_POST['_polar_product_id'] ) ) {
			$polar_product_id = sanitize_text_field( wp_unslash( $_POST['_polar_product_id'] ) );
